...
def _store_verification_code(session: Session, email: str, scene: VerificationScene, code: str) -> EmailVerificationCode:
    session.exec(
...
def _verify_email_code(session: Session, email: str, scene: VerificationScene, code: str) -> None:
    code_hash = _hash_code(code)
    statement = (
...
async def send_email_code(payload: EmailCodeSendRequest, session: SessionDep):
    ...
    verification = _store_verification_code(session, email, scene, code)
